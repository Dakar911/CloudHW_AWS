...
cd ../resize
sudo docker build -t resize:latest .
cd ../grayscale
sudo docker build -t grayscale:latest .
cd ../detect
...
kubectl apply -f k8s/imagegrab.yaml
kubectl apply -f k8s/resize.yaml
kubectl apply -f k8s/grayscale.yaml
kubectl apply -f k8s/detect.yaml
kubectl apply -f k8s/tag.yaml
```
Logok
```
kubectl logs deployment/imagegrab
kubectl logs deployment/resize
kubectl logs deployment/grayscale
kubectl logs deployment/detect
kubectl logs deployment/tag
```
Check
```
kubectl get pods
